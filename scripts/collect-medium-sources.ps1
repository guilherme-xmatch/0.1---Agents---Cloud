param(
    [string]$OutputPath = (Join-Path $env:TEMP 'claude-code-research\community\medium-manifest.json'),
...
$feeds = @(
    @{ label = 'tag:claude-code'; url = 'https://medium.com/feed/tag/claude-code'; focus = 'direct' },
    @{ label = 'tag:claude-ai'; url = 'https://medium.com/feed/tag/claude-ai'; focus = 'adjacent' },
    @{ label = 'tag:anthropic'; url = 'https://medium.com/feed/tag/anthropic'; focus = 'adjacent' },
    ...
    @{ label = 'tag:coding-agent'; url = 'https://medium.com/feed/tag/coding-agent'; focus = 'adjacent' },
...
function Get-RelevanceClass {
    ...
    $normalized = ($Text | ForEach-Object { if ($_){ $_.ToLowerInvariant() } else { '' } })
    ...
    try {
        ...
        foreach ($item in $xml.SelectNodes('//item')) {
            ...
            $record = [ordered]@{
                ...
                published = $published
                modified = $modified
                status = if ($effectiveDate) { if ($effectiveDate -ge $cutoffDate) { 'verified-after-cutoff' } else { 'verified-before-cutoff' } } else { 'date-unverified' }
                relevance = $relevance
...
$dedup = $rawItems |
    ...
    ForEach-Object {
        $_.Group |
            Sort-Object @{ Expression = { if ($_.status -eq 'verified-after-cutoff') { 3 } elseif ($_.status -eq 'date-unverified') { 2 } elseif ($_.status -eq 'verified-before-cutoff') { 1 } else { 0 } }; Descending = $true },
...
$manifest.items |
    ...
    Select-Object -First 15 title,effectiveDate,url |
